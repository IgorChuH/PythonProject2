...
import pandas as pd
from src.reports import spending_by_category  # замените your_module на имя файла с функцией
...
class TestSpendingByCategory(unittest.TestCase):
    ...
    def test_spending_with_fixed_date(self, mock_getLogger, mock_datetime):
        ...
        mock_datetime.strptime.side_effect = lambda s, f: real_datetime.datetime.strptime(s, f)
        ...
        mock_logger.debug.assert_any_call("Преобразовали столбец 'Дата платежа' в datetime")
    ...
    def test_spending_with_invalid_date_logs_error(self, mock_getLogger):
        ...
        self.assertIn('Ошибка при расчете трат по категории', mock_logger.error.call_args[0][0])
